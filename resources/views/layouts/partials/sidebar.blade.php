<div class="h-full flex flex-col font-sans transition-colors duration-500">
    ...
    <nav class="flex-1 overflow-y-auto px-4 py-2 space-y-8 custom-scrollbar pb-10">
        ...
        <!-- Section: Domain Center -->
        <div x-data="{ domainOpen: {{ request()->routeIs('domain-center.*') ? 'true' : 'false' }} }">
            <button @click="domainOpen = !domainOpen"
                ...
                <svg x-show="!sidebarCollapsed" class="w-4 h-4 transition-transform duration-200" :class="domainOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
            </button>
            <div x-show="domainOpen"
                 ...
                 class="mt-1 mr-4 space-y-1 border-r-2 border-indigo-200 dark:border-indigo-800 pr-3">
                <a href="{{ route('domain-center.domains') }}"
                   class="flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all
                   {{ request()->routeIs('domain-center.domains') ? 'text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                    <span x-show="!sidebarCollapsed">دامنه‌های اختصاصی</span>
                </a>
                <a href="{{ route('domain-center.connect') }}"
                   class="flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all
                   {{ request()->routeIs('domain-center.connect') ? 'text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                    <span x-show="!sidebarCollapsed">اتصال دامنه</span>
                </a>
                <a href="{{ route('domain-center.parked') }}"
                   class="flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all
                   {{ request()->routeIs('domain-center.parked') ? 'text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                    <span x-show="!sidebarCollapsed">پارک دامنه</span>
                </a>
                <a href="{{ route('domain-center.ns-settings') }}"
                   class="flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all
                   {{ request()->routeIs('domain-center.ns-settings') ? 'text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
            ...
            <ul class="space-y-1.5">
                ...
                <li>
                    <a href="{{ route('security.index') }}"
                       class="group flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold transition-all
                       {{ request()->routeIs('security.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/25' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white' }}">
                        <svg class="w-5 h-5 opacity-60 group-hover:opacity-100 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                        </svg>
                        <span x-show="!sidebarCollapsed">مرکز امنیت</span>
                    </a>
                </li>
                <li>
