...
<!-- Notification System -->
<div x-data="notificationSystem()" x-on:notify.window="notify($event)" class="fixed right-3 top-3 z-50 w-80 max-w-[90vw] space-y-2" style="z-index: 9999">
    <template x-for="item in items" :key="item.id">
        <div x-show="true" x-transition class="rounded-2xl px-4 py-3 text-sm shadow-lg border backdrop-blur bg-white/90 dark:bg-gray-900/90"
             :class="{
            ...
            <div class="flex items-start justify-between gap-3">
                ...
                <button type="button" class="text-xs opacity-60 hover:opacity-100" @click="remove(item.id)">✕</button>
            </div>
        </div>
    </template>
</div>
...
<div x-data="dashboardLayout()" x-init="init()" class="min-h-dvh flex">
    ...
    <div x-show="mobileOpen" x-cloak class="lg:hidden" x-ref="dialog" aria-modal="true">
        <div x-show="mobileOpen" x-transition:enter="transition-opacity ease-linear duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-900/80 z-40"></div>
        <div class="fixed inset-0 flex z-50">
            <div x-show="mobileOpen" @click.outside="mobileOpen = false" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" class="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white dark:bg-gray-800 pb-12 shadow-xl">
    ...
    function notificationSystem() {
        return {
            ...
            notify(e) {
                ...
                this.items.push({ id, type: detail.type || 'info', text: detail.text || detail.message || '' });
