                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    @if($clientDomains->isNotEmpty())
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-emerald-600 dark:text-emerald-400 block mb-1">
                                                دامنه اختصاصی مشتری:
                                            </span>
                                            @foreach($clientDomains as $mapping)
                                                <a href="https://{{ $mapping->source_domain }}" target="_blank" class="text-sm font-bold text-emerald-600 dark:text-emerald-400 hover:underline inline-flex items-center gap-1 truncate w-full" dir="ltr" title="دامنه مشتری">
                                    ...
                                    <span class="text-xs text-gray-500 dark:text-gray-400 block mb-1">
                                        {{ $clientDomains->isNotEmpty() ? 'دامنه پایه سرویس (مقصد):' : 'دامنه / آدرس سرویس:' }}
                                    </span>
                                    @if($service->type == 'subfolder')
                                        <a href="http://{{ env('APP_MAIN_DOMAIN', 'localhost') }}/{{ $service->domain }}" target="_blank" class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center gap-1 truncate w-full" dir="ltr">
                                    ...
                                    @else
                                        <a href="http://{{ $service->domain }}" target="_blank" class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center gap-1 truncate w-full" dir="ltr">
                                            {{ $service->domain }}
                                            <svg class="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                        </a>
                                    @endif
